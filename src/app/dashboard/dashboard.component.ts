...
export class DashboardComponent implements OnInit {
	...
	public isAuth = () => (this.auth.isAuth ? this.auth.isAuth : null);
  	GetDynamicGlobalProperties (){
		this._http.get('https://api.steemfollower.com/api/database/get_dynamic_global_properties')
		.toPromise()
		.then(
	...
	Profile_about(){
 	...
 	Profile_reputation () {
		return (
			this.auth.Reputaionformatter(this.User().account.reputation) ?
			 	this.auth.Reputaionformatter(this.User().account.reputation) :
			  null
		);
 	}
	...
	Profile_votingpower() {
		...
		let powernow = parseFloat(power.toFixed(2));
		if(powernow > 100){
			powernow = 100;
		}
	...
	Account_sp(){
		if(this.User() && this.User().account && this.DynamicGlobalProperties){
			...
			let sp = (vesting * (tvfs/tvs)).toFixed(3).toString();
			let spr = (received * (tvfs/tvs));
			let spd = (delegated * (tvfs/tvs));
